...
export const kbArticles: KbArticle[] = [
  ...
  },
  {
    ...
    body: [
      ...
      'SNP family — AHIP start here: A SNP (Special Needs Plan) is a Medicare Advantage plan limited to a CMS-defined special-needs population. Confirm Hood County / ZIP availability and the client’s qualifying status before naming one. Exam trap: a regular MA plan with many dual members is not a SNP unless CMS designated it as one.',
      'Core SNP types: C-SNP — Chronic Condition SNP (must have a qualifying chronic condition on the current CMS list). D-SNP — Dual Eligible SNP (Medicare + Medicaid; Medicaid level still matters). I-SNP — Institutional SNP (usually resides in a long-term care facility). IE-SNP — Institutional Equivalent SNP (lives in the community but needs an institutional level of care).',
      'SNP integration and look-alikes (duals — common exam items): FIDE SNP — Fully Integrated Dual Eligible (Medicare + full Medicaid through one highly integrated plan). HIDE SNP — Highly Integrated Dual Eligible (substantial Medicaid integration, not full FIDE). AIP — Applicable Integrated Plan (CMS label for certain D-SNPs that must coordinate with Medicaid). MMP — Medicare-Medicaid Plan (state/CMS combined plan; do not assume one exists in Texas). D-SNP look-alike — regular MA with many dual members that is not a D-SNP and has no SNP Medicaid-coordination duties. Look-alike ≠ D-SNP.',
